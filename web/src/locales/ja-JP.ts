export default {
  ...
  keys: {
    ...
    upstreamTooltip:
      "APIサーバーの完全なURL。ドメインのみ、バージョンパス（例: /v1）、または完全なエンドポイントパス（例: /v1/chat/completions）のいずれかを入力でき、自動的に正規化されます。複数のアップストリームにより高可用性のためのロードバランシングとフェイルオーバーが可能",
    urlMode: "URLモード",
    urlModeHost: "ホストのみ",
    urlModePrefix: "プレフィックスパス",
    urlModeFull: "完全パス",
    urlModeTooltip: "アップストリームURLとリクエストパスの結合方法を決定します",
    urlModeHostTip:
      "ホストのみ：アップストリームURLはホスト名として扱い、リクエストパスをそのまま追加します（デフォルト）",
    urlModePrefixTip:
      "プレフィックスパス：アップストリームURLをパスプレフィックスとして扱い、リクエストパスと重複する先頭セグメントを自動的に削除します",
    urlModeFullTip:
      "完全パス：アップストリームURLを完全なエンドポイントとして直接使用し、リクエストパスを無視します",
    weightTooltip:
